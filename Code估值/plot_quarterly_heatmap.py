# -*- coding: utf-8 -*-
"""PriceSW1 季度收益率热力图（2022-01-01至今）"""
...
for q,g in d.groupby('Q'):
    ...
    rv['Quarter']=str(q);rows.append(rv)
dr=pd.DataFrame(rows).set_index('Quarter').T
dr.index=dr.index.map(lambda x: str(x).split('\n')[0].replace('(申万)','').strip())
dr=reindex_by_sector(dr, 'sw')
print(f"矩阵: {dr.shape[0]}行业 x {dr.shape[1]}季度")
print("季度:",list(dr.columns))
print(f"范围: {dr.min().min():.1f}%~{dr.max().max():.1f}%")

ex=os.path.join(out_dir,'申万一级行业季度收益率矩阵.xlsx')
dr.to_excel(ex);print(f"导出: {ex}")

nc=len(dr.columns);fw=max(12,nc*0.9*FONT_SCALE);fh=max(9,dr.shape[0]*0.4*FONT_SCALE)
plt.figure(figsize=(fw,fh))
sns.heatmap(dr,annot=True,fmt=".1f",cmap='RdYlGn_r',linewidths=0.5,center=0,
    cbar_kws={'label':'季度收益率(%)'},annot_kws={'size':max(9,11*FONT_SCALE),'weight':'bold'})
plt.title('申万一级行业季度收益率热力图 (2022-至今)',fontsize=max(13,15*FONT_SCALE),fontweight='bold',pad=15)
plt.xlabel('季度',fontsize=12*FONT_SCALE,labelpad=10)
plt.ylabel('申万一级行业',fontsize=12*FONT_SCALE,labelpad=10)
plt.xticks(rotation=45 if nc>6 else 0,fontsize=max(9,11*FONT_SCALE),fontweight='bold')
plt.yticks(fontsize=max(9,11*FONT_SCALE),fontweight='bold',rotation=0)
plt.tight_layout()
cp=os.path.join(out_dir,'申万一级行业季度收益率热力图.png')
