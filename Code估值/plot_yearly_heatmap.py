...
print(f"数据: {len(d)}行 {len(ind)}行业")

d['Year']=d['Date'].dt.year
# 剔除2015年（仅含1个交易日，用于2016年基线计算）
d=d[d['Year']>=2016].reset_index(drop=True)
rows=[]
for yr,g in d.groupby('Year'):
    g=g.sort_values('Date');f=g.iloc[0];l=g.iloc[-1]
    rv={i:(l[i]/f[i]-1)*100 for i in ind if pd.notna(f[i]) and f[i]>0 and pd.notna(l[i])}
    rv['Year']=str(yr);rows.append(rv)
dr=pd.DataFrame(rows).set_index('Year').T
dr.index=dr.index.map(lambda x: str(x).split('\n')[0].replace('(申万)','').strip())
dr=reindex_by_sector(dr, 'sw')
print(f"矩阵: {dr.shape[0]}行业 x {dr.shape[1]}年")
print("年份:",list(dr.columns))
print(f"范围: {dr.min().min():.1f}%~{dr.max().max():.1f}%")

ex=os.path.join(out_dir,'申万一级行业年度收益率矩阵.xlsx')
dr.to_excel(ex);print(f"导出: {ex}")

nc=len(dr.columns);fw=max(10,nc*1.2*FONT_SCALE);fh=max(8,dr.shape[0]*0.38*FONT_SCALE)
plt.figure(figsize=(fw,fh))
sns.heatmap(dr,annot=True,fmt=".1f",cmap='RdYlGn_r',linewidths=0.5,center=0,
    cbar_kws={'label':'年度收益率(%)'},annot_kws={'size':max(9,11*FONT_SCALE),'weight':'bold'})
plt.title('申万一级行业年度收益率热力图 (2016-2026)',fontsize=max(13,15*FONT_SCALE),fontweight='bold',pad=15)
plt.xlabel('年份',fontsize=12*FONT_SCALE,labelpad=10)
plt.ylabel('申万一级行业',fontsize=12*FONT_SCALE,labelpad=10)
plt.xticks(rotation=0,fontsize=max(9,11*FONT_SCALE),fontweight='bold')
plt.yticks(fontsize=max(9,11*FONT_SCALE),fontweight='bold',rotation=0)
plt.tight_layout()
cp=os.path.join(out_dir,'申万一级行业年度收益率热力图.png')
