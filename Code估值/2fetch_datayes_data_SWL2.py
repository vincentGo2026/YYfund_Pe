...
def fetch_datayes_secondary_industry_data():
    ...
    for index, row in ind_df.iterrows():
        ...
        print(f"正在获取二级行业 [{name} - {ticker}] 数据 ...")
    ...
    if all_data:
        ...
        # Exporting to CSV
        output_path = rf"D:\CC\DB\data\datayes_all_SW_Industries_Level2_{begin_date}_{end_date}.csv"
        final_df.to_csv(output_path, index=False, encoding='utf-8-sig')
        print(f"\n[SAVED] 所有二级行业数据已成功保存至: {output_path} (共 {len(final_df)} 行)")
    else:
        print("\n[ERROR] 未能成功获取任何二级行业数据。")

if __name__ == "__main__":
    fetch_datayes_secondary_industry_data()
