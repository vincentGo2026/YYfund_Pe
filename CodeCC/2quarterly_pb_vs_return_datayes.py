...
输出：
  - D:\CC\Mid\估值\REStemp\  每个一级行业一个 .xlsx 文件（PB版），每个二级行业一个 sheet
"""
...
OUT_DIR    = r"D:\CC\Mid\估值\REStemp\pb"
OUT_CSV    = r"D:\CC\Mid\估值\REStemp\pb\quarterly_pb_vs_return_data_v2.csv"
...
df_val = df_val[["tradeDate", "pb", "Fetched_Ind_Name", "申万一级行业"]].copy()
...
for l1_name, grp_l1 in df_result.groupby("一级行业"):
    ...
    for l2_name, grp_l2 in grp_l1.groupby("二级行业"):
        ...
        ax.set_xlabel("季度末 PB (市净率)", fontsize=11)
        ax.set_ylabel("次季度收益率 (%)", fontsize=11)
        ax.set_title(
            f"{l1_name} · {l2_name} — 季度末 PB vs 次季度收益率",
        ...
        data_start_col = 16
        headers = ["季度(YYYYMM)", "季度末PB", "次季度收益率(%)"]
        ...
        for i, (_, row) in enumerate(grp_plot.iterrows()):
            ...
            ws.cell(row=i + 2, column=data_start_col + 1, value=row["季度末PB"])
