# -*- coding: utf-8 -*-
"""柱状图(最新PB)+折线图(百分位). 字号 FONT_SCALE 整体缩放"""
...
def draw_dual(data, name_col, title, filename, bar_col='最新PB', pct_col='PB历史百分位(%)', top_n=None):
    d = data.sort_values(bar_col, ascending=False).reset_index(drop=True)
    if top_n:
        d = d.head(top_n)
    n = len(d)
    fig_w = max(12, n * 0.35 * FONT_SCALE)
    fig_h = max(6, n * 0.25 * FONT_SCALE)
    fig, ax1 = plt.subplots(figsize=(fig_w, fig_h))
    names = d[name_col].tolist()
    vals = d[bar_col].values
    pcts = d[pct_col].values
    y = range(n)
    bh = 0.7 * FONT_SCALE
    vfs = max(fs(7), 9)
    nfs = max(fs(7), 9)
    ...
    ax1.set_xlabel('最新PB', fontsize=fs(11), color='#2c3e50')
    ax1.xaxis.set_major_formatter(mticker.FormatStrFormatter('%.2f'))
    ax1.tick_params(axis='x', colors='#2c3e50')
    for i, v in enumerate(vals):
        ax1.text(v + vals.max() * 0.005, i, f'{v:.2f}', va='center', fontsize=vfs, color='#2c3e50')
    ax2 = ax1.twiny()
    ...
    ax2.set_xlabel('PB历史百分位(%)', fontsize=fs(11), color='#e74c3c')
    ...
    ax1.set_yticklabels(names, fontsize=nfs)
    ax1.invert_yaxis()
...
draw_dual(
    pd.read_excel('PB_Statistics_by_Sector.xlsx', sheet_name='PB_PB统计'),
    '申万二级行业_API名',
    '申万二级行业 最新PB及历史百分位（2021-2026）',
    'Chart_PB_L2_BarWithLine.png',
    top_n=40,
)
...
raw = raw[raw['pb'] > 0]
l, h = raw['pb'].quantile(0.01), raw['pb'].quantile(0.99)
raw = raw[(raw['pb'] >= l) & (raw['pb'] <= h)]


def calc_l1_pb(d):
    lt = d.groupby('申万一级行业').apply(
        lambda g: g.sort_values('tradeDate').iloc[-1]['pb'], include_groups=False
    ).reset_index()
    lt.columns = ['申万一级行业', '最新PB']
    pc = d.groupby('申万一级行业').apply(
        lambda g: (g['pb'] < g.sort_values('tradeDate').iloc[-1]['pb']).sum() / len(g) * 100,
        include_groups=False,
    ).reset_index()
    pc.columns = ['申万一级行业', 'PB历史百分位(%)']
    return lt.merge(pc, on='申万一级行业').sort_values('最新PB', ascending=False).reset_index(drop=True)


l1 = calc_l1_pb(raw)
l1['PB历史百分位(%)'] = l1['PB历史百分位(%)'].round(2)
draw_dual(
    l1,
    '申万一级行业',
    '申万一级行业 最新PB及历史百分位（2021-2026）',
    'Chart_PB_L1_BarWithLine.png',
)
