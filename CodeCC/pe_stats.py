...
before = len(df)
df = df[df['pe'] > 0]
print(f"删除 {before - len(df)} 行 pe<=0 的数据")
...
# ============================================================
stats = df.groupby('secShortName')['pe'].agg([
    ('最大值', 'max'),
    ('最小值', 'min'),
    ('中位数', 'median'),
...
# 4. 导出 Excel
# ============================================================
excel_path = r"D:\CC\Mid\估值\PE_Statistics_by_Sector.xlsx"
with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
    stats.to_excel(writer, sheet_name='PE统计', index=True)
    ws = writer.sheets['PE统计']
    ...
    info_df = pd.DataFrame({'说明': [
        '数据文件: datayes_all_SW_Industries_Level2_mapped_20210501_20260521.csv',
...
ax.invert_yaxis()
ax.set_xlabel('PE（市盈率）', fontsize=12)
ax.set_title('申万二级行业 PE 中位数及四分位区间（2021-2026）', fontsize=14, fontweight='bold')
ax.xaxis.set_major_formatter(mticker.FormatStrFormatter('%.1f'))

for i, v in enumerate(medians):
    ax.text(v + 0.3, i, f'{v:.1f}', va='center', fontsize=7.5, color='darkred')

plt.tight_layout()
p1 = r"D:\CC\Mid\估值\Chart_PE_Median_Quartiles.png"
...
ax.invert_yaxis()
ax.set_xlabel('PE（市盈率）', fontsize=12)
ax.set_title('申万二级行业 PE 平均值（2021-2026）', fontsize=14, fontweight='bold')
ax.xaxis.set_major_formatter(mticker.FormatStrFormatter('%.1f'))
for i, v in enumerate(means):
    ax.text(v + 0.3, i, f'{v:.1f}', va='center', fontsize=7.5, color='darkred')
plt.tight_layout()
p2 = r"D:\CC\Mid\估值\Chart_PE_Mean_Bar.png"
...
ax.set_title('申万二级行业 PE 多项指标对比（Top 15）', fontsize=14, fontweight='bold')
ax.legend(fontsize=9)
ax.yaxis.set_major_formatter(mticker.FormatStrFormatter('%.1f'))
plt.tight_layout()
p3 = r"D:\CC\Mid\估值\Chart_PE_Multi_Metrics.png"
...
ax.axvline(all_m.mean(), color='red', ls='--', lw=2, label=f'均值={all_m.mean():.1f}')
ax.axvline(np.median(all_m), color='green', ls='--', lw=2, label=f'中位数={np.median(all_m):.1f}')
ax.set_xlabel('PE 中位数', fontsize=12)
ax.set_ylabel('行业数', fontsize=12)
ax.set_title('申万二级行业 PE 中位数分布', fontsize=14, fontweight='bold')
ax.legend(fontsize=10)
plt.tight_layout()
p4 = r"D:\CC\Mid\估值\Chart_PE_Median_Distribution.png"
